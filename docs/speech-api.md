...
> ⚠️ **冻结参考（2026-09-04）**：`:speech` 模块**未被 :app 依赖**（settings 仍 include，供未来集成参考；不进 APK）。
> App 语音功能已于 08-27 按用户决定全量下线；离线中文语音的接入手册见 `D:\work2\android-local-speech-recognition\HANDOVER.md`。
