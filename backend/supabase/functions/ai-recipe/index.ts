...
规则:
1. 【硬性】每道菜必须至少吃掉 1 种 uses 列出的临期食材; 主料必须全部来自用户现有食材(严禁"须补充主料"的菜: 没有虾滑就不得做虾滑菜), 仅葱姜蒜等家常调料可标"需补充"; 30 道尽量风格/食材组合多样化
...
规则:
1. 【硬性】主料必须全部来自用户列出的现有食材! 严禁用用户没有的主料做菜(用户没有蒜苔就不得做蒜苔菜, 没有虾滑就不得做虾滑菜, 没有藕就不得做藕片菜); 只有葱姜蒜/油盐酱醋糖等家常调料允许标"需补充:xxx"; 缺主料的菜一律不得出现; 30 道风格尽量不同(炒/汤/主食/凉拌/烧烩都安排)
2. 若库存存在临期食材: 至多 10 道以临期食材为主料(其余自由搭配); 若不存在临期食材: 全部正常自由搭配(不生成"必须消耗临期食材"的菜)
3. 步骤用中文、简洁、可按顺序执行
4. 只输出 JSON, 不要解释; 若接口要求对象形式, 同样按上面结构返回
5. 【求新·最高优先】本批基调=__MOOD__。全部菜围绕该基调；与【上次已做过的菜】相比: 最多允许 2 道同名且必须换做法/配菜, 其余必须为全新菜品(同名不同法也算新)；绝不允许整批与上次相同
